...
export async function createInitialLayoutAction(): Promise<{ success: boolean; html?: string; editionNumber?: number; error?: string }> {
    ...
    try {
        ...
        // Retrieve sports boxes generated by Sports Journalist
        let sportsBoxes: any[] = [];
        try {
            const sportsDataDoc = await getDoc(doc(firestore, 'sports_data', 'current'));
            if (sportsDataDoc.exists()) {
                const sportsData = sportsDataDoc.data();
                sportsBoxes = sportsData.boxes || [];
                console.log(`📊 Editor 1 [${editionCreationId}]: Retrieved ${sportsBoxes.length} sports boxes from Sports Journalist`);
            }
        } catch (error) {
            console.warn(`⚠️ Editor 1 [${editionCreationId}]: No sports data found`);
        }
        
        const layout = await generateNewspaperLayout({ articles, editionNumber: newEditionNumber, sportsBoxes });
    ...
    try {
        ...
        try {
            ...
            if (sportsDataDoc.exists()) {
                const sportsData = sportsDataDoc.data();
                sportsBoxes = sportsData.boxes || [];
                console.log(`📊 Editor [${editionCreationId}]: Retrieved ${sportsBoxes.length} sports boxes from Sports Journalist`);
            } else {
                console.warn(`⚠️ Editor [${editionCreationId}]: No sports data found. Sports page may be empty.`);
            }
        } catch (error) {
            console.error(`❌ Editor [${editionCreationId}]: Failed to retrieve sports data:`, error);
        }
